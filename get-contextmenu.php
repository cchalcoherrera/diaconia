...
if (isset($_GET['ide'])) {
    ...
    if ($quote === FALSE) {
                    ...
                    $sql .= "and not exists( select 
                        ...
                        where
                            sdf2.id_emision = sde.id_emision
                                and sde.cobertura != 2 )";
                } elseif ($token !== 5 && $token !== 3 && $token !== 4 && $token !== 2 && $token !== 6 && $token !== 7) {
        ...
        if (($rs = $link->query($sql, MYSQLI_STORE_RESULT))) {
            if ($rs->num_rows === 1) {
                ...
                if ($cp === true) {
                    ...
                    if ($row['id_prcia'] == 7 and $row['fecha_suma'] >= "2017-02-01") {
                        if ($row['fecha_creacion'] < '2017-02-01') {
                            $category = base64_encode('CE-HIP-ANT');
                        } else {
                ...
                if ($product === 'DE' && $bc === 2) {
                    ...
                    if (($rsDt = $link->query($sqlDt, MYSQLI_STORE_RESULT)) !== false) {
                        if ($rsDt->num_rows > 0) {
                                                ...
                                                if ($token === 1) {
                                                    switch ($arr_p['id_estado']) {
                                                        case 3:
                                                            ...
                                                            $arr_state['link'] = 'fac-de-observation.php?ide='
                                                        ...
                                                        case 4:
                                                            ...
                                                            //$row['observacion'] = 'E';
                                                            $arr_state['link'] = 'de-quote.php?ms=&page=&pr='
                            ...
                            if ($token === 4 && (boolean) $row['emitir'] === true && (boolean) $row['anulado'] === false) {
                                $arr_state['action'] = 'Anular Certificado';
                                $arr_state['link'] = 'cancel-policy.php?ide='
                                        . base64_encode($row['ide'])
                                        . '&pr=' . base64_encode($product);
                            } elseif ($token === 3 && (boolean) $row['emitir'] === false && (boolean) $row['facultativo'] === false && (boolean) $row['aprobado'] === false && (boolean) $row['anulado'] === false) {
                ...
                if ($token === 1) {
                    ...
                    switch ((int) $row['leido']) {
                        case 1: $txtMark = 'Marcar como no Leído';
                            break;
                        case 0: $txtMark = 'Marcar como Leído';
                            break;
                    }
        ...
        if ($token !== 3 && $token !== 7) {
            ...
            $menu .= '<li><a href="de-quote.php?EDIT=edit&ms='.md5('MS_DE').'&ide=' . base64_encode($ide) . '&pr=' . base64_encode('DE|05') . '&type=' . base64_encode('PRINT') . '&category=' . $category . '&idc=' . base64_encode($idc) . '&flag='.md5('i-edit').'&cia=' . base64_encode($row['id_compania']) . '">Editar</a></li>';
                ...
                if ($rs->num_rows === 1) {
                    ...
                    if ($token === 3) {
                        if ($limit === 0) {
                            ...
                            $menu .= '<li><span class="cm-link"><span class="view-ste">FECHA L�?MITE DE EMISIÓN CADUCADA</span></span></li>';
                        }
                    }
                }
            }
