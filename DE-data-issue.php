...
if (($data = $PolicyController->getPolicyData($sw, $idc, $ide, $_SESSION['idEF'])) !== false) {
    ...
    if ($n_cl <= $max_item) {
        ...
        <a href="certificate-detail.php?idc=<?= base64_encode($data[0]['id_cotizacion']); ?>&cia=<?= $_GET['cia']; ?>&type=<?= base64_encode('PRINT'); ?>&pr=<?= base64_encode('DE'); ?>" 
        ...
        <form id="fde-issue" name="fde-issue" action="" method="post" class="form-quote form-customer">
            ...
            foreach ($data as $key => $row):
                ...
                if (($cr_currency === 'BS' && $cr_amount > 35000) || ($cr_currency === 'USD' && $cr_amount > 5000)) {
                    $swDE = true;
                }
                ...
                <div class="form-col">
                    ...
                    <div class="content-input">
                        <select id="dc-<?= $cont; ?>-hand" name="dc-<?= $cont; ?>-hand" 
                            ...
                            if ($cl_hand == "DE") {
                                ?>
                                <option value="DE" selected>Derecha</option>  
                                <option value="IZ">Izquierda</option> 
                                <?php
                            } elseif ($cl_hand == "IZ") {
                                ?>
                                <option value="DE">Derecha</option>  
                                <option value="IZ" selected>Izquierda</option> 
                                <?php
                            } elseif ($cl_hand == "") {
                                ?>
                                <option value="DE">Derecha</option>  
                ...
                --><div class="form-col">
                    ...
                    <textarea id="dc-<?= $cont; ?>-address-home" name="dc-<?= $cont; ?>-address-home" 
                              class="fbin" <?= $read_new; ?>><?= $row['cl_direccion']; ?></textarea><br>
                    ...
                    <div class="content-input">
                        <script type="text/javascript">
                            $(document).ready(function (e) {
                                ...
                                $("#dc-<?= $cont; ?>-edit").click(function () {

                                    $('#dc-<?= $cont; ?>-desc-occ').val('');
                                    $('#dc-<?= $cont; ?>-carga').show();
                                    $('#dc-<?= $cont; ?>-occupation').hide();
                                    $.ajax({
                                        type: "POST",
                                        url: "getOcupacion.php?count=<?= $cont; ?>",
                                        success: function (response)
                                        {
                                            $('#dc-<?= $cont; ?>-occupation').html(response).fadeIn();
                                            $('#dc-<?= $cont; ?>-carga').hide();
                                            $('#dc-<?= $cont; ?>-occupation').show();
                                        }
                                    });
                                });
                            });
                        </script><div id="dc-<?= $cont; ?>-edit"><img src="img/edit-inf-icon.png" width="25" height="25" alt="Editar" title="Editar" style="float:left; padding-top: 5px; cursor: pointer; display:<?= $show; ?>" ></div>
                        <div id="dc-<?= $cont; ?>-carga" style="float: left;">
                        ...
                        <select id="dc-<?= $cont; ?>-occupation" name="dc-<?= $cont; ?>-occupation" 
                                class="fbin occupation" <?= $read_save; ?> style="width: 150px;">
                            <option value="">Seleccione...</option>
                            <?php
                            $ocupacion = $ClientController->getOccupationCaedecCustomer($row['cl_ocupacion']);
                            $selected = ''
                            ?>
                            <option selected="selected" value="<?= base64_encode($ocupacion[0]['id_ocupacion']); ?>" 
                                    ...
                                    >Código CAEDEC - <?= $ocupacion[0]['codigo']; ?></option>
                        </select>
                    </div><br>
            ...
            <?php
            if ($cr_product == 7 and $cr_fecha_suma >= "2017-02-01") {
                $functionName = "getQuestionHipotecario";
            } else {
                $functionName = "getQuestion";
            }
            if (($questions = $QuestionController->$functionName($_SESSION['idEF'])) !== false):
                $resp = array();
                $required = array();
                $fac = array();
                ?>
            ...
            créditos adicionales, FUERA de la línea.';
                ...
                $arr_sub_title = array('Titular', 'Deudor', 'Codeudor', 'Codeudor', 'Codeudor', 'Codeudor', 'Codeudor', 'Codeudor');
                $sub_title = '';
                $percentage = 0;
            ...
            <input type="hidden" id="id_prcia" name="id_prcia" value="<?= $cr_product; ?>" >
            <input type="hidden" id="edit" name="edit" value="<?php if(isset($_GET['EDIT'])){echo $_GET['EDIT'];}; ?>">
