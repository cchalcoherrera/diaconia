    ...
    if (isset($_POST['accionGuardar'])) {
        ...
        while ($t <= $numerotasas) {
            ...
            if ($t == 1) {//COLUMNA TASA COMPAÑIA
                ...
                while ($tc <= $_POST['num_prod']) {
                    ...
                    $valor_tc_mancomunado = $_POST["txtTcompaniaMancomunado" . $tc];
                ...
                while ($tf <= $_POST['num_prod']) {
                    ...
                    $numtfmancomunado = validar_numero($valor_tf_mancomunado);
        ...
        if ($errFlag) {
            ...
            while ($j <= $_POST['num_prod']) {
                ...
                $update = "UPDATE s_tasa_de SET tasa_cia=" . $tasacompania . ",tasa_cia_mancomunado=" . $tasacompaniamancomunado . ",  tasa_banco=" . $tasabanco . ", tasa_final=" . $tasafinal . ", tasa_final_mancomunado=" . $tasafinalmancomunado . " WHERE id_tasa=" . $idtasa . " and id_prcia=" . $idprcia . ";";
...
function mostrar_editar_tasas($id_usuario_sesion, $tipo_sesion, $usuario_sesion, $conexion, $errArr) {
    ...
    $selectTs = "select 
				  ...
				  td.id_tasa,
				  tasa_cia,
                                  tasa_cia_mancomunado,
				  tasa_banco,
				  tasa_final,
                                  tasa_final_mancomunado
				from
...
<div class="da-panel collapsible" style="width:780px;">
    ...
    if ($num > 0) {
						...
						<thead>
							<tr>
								...
								<th><b><span lang="es">Tasa Compañía</span></b></th>
                                                                <th><b><span lang="es">Tasa Compañía(Mancomunado)</span></b></th>
								<th><b><span lang="es">Tasa Banco</span></b></th>
								<th><b><span lang="es">Tasa Final Normal</span></b></th>
                                                                <th><b><span lang="es">Tasa Final Macomunado</span></b></th>
								<th>&nbsp;</th>
							</tr>
						</thead>
						<tbody>';
        ...
        $i = 1;
        $suma_total = 0;
        $suma_total_mancomunado = 0;
        while ($regi = $resu->fetch_array(MYSQLI_ASSOC)) {
            if (isset($_POST["txtTcompania" . $i]))
                $tcompania = $_POST["txtTcompania" . $i];
            else
                $tcompania = $regi['tasa_cia'];
            if (isset($_POST["txtTcompaniaMancomunado" . $i]))
                $tcompaniaMancomunado = $_POST["txtTcompaniaMancomunado" . $i];
            else
                $tcompaniaMancomunado = $regi['tasa_cia_mancomunado'];
            if (isset($_POST["txtTbanco" . $i]))
                $tbanco = $_POST["txtTbanco" . $i];
            else
                $tbanco = $regi['tasa_banco'];
            if (isset($_POST["txtTfinal" . $i]))
                $tfinal = $_POST["txtTfinal" . $i];
            else
                $tfinal = $regi['tasa_final'];
            $suma_total = $tcompania + $tbanco;
            if (isset($_POST["txtTfinalMancomunado" . $i]))
                $tfinalmancomunado = $_POST["txtTfinalMancomunado" . $i];
            else
                $tfinalmancomunado = $regi['tasa_final_mancomunado'];
            $suma_total_mancomunado = $tcompaniaMancomunado + $tbanco;
            echo'<tr>
											<td>' . $i . '</td>
											<td>' . $regi['nombre'] . '</td>
											<td><input type="text" name="txtTcompania' . $i . '" id="' . $i . '-txtTcompania" value="' . $regi['tasa_cia'] . '" class="validatasa"/><span class="errorMessage" id="errortcia' . $i . '"></span></td>
                                                                                        <td><input type="text" name="txtTcompaniaMancomunado' . $i . '" id="' . $i . '-txtTcompaniaMancomunado" value="' . $regi['tasa_cia_mancomunado'] . '" class="validatasa"/><span class="errorMessage" id="errortcia' . $i . '"></span></td>
											...
											<td><input type="text" name="txtTfinal' . $i . '" id="' . $i . '-txtTfinal" value="' . $suma_total . '" readonly="readonly" class="validatasa"/>
                                                                                            <td><input type="text" name="txtTfinalMancomunado' . $i . '" id="' . $i . '-txtTfinalMancomunado" value="' . $suma_total_mancomunado . '" readonly="readonly" class="validatasa"/>
        ...
        while ($regi = $resu->fetch_array(MYSQLI_ASSOC)) {
            ...
            echo'<tr>
											...
											<td><input type="text" name="txtTcompania' . $i . '" id="' . $i . '-txtTcompania" value="' . $tcompania . '" class="validatasa"/><span class="errorMessage" id="errortcia' . $i . '"></span></td>
                                                                                        <td><input type="text" name="txtTcompaniaMancomunado' . $i . '" id="' . $i . '-txtTcompaniaMancomunado" value="' . $tcompaniaMancomunado . '" class="validatasa"/><span class="errorMessage" id="errortcia' . $i . '"></span></td>
											...
											<td><input type="text" name="txtTfinal' . $i . '" id="' . $i . '-txtTfinal" value="' . $suma_total . '" readonly="readonly" class="validatasa"/><input type="hidden" name="idprcia' . $i . '" id="idprcia' . $i . '" value="' . $regi['id_prcia'] . '"/></td>
                                                                                        <td><input type="text" name="txtTfinalMancomunado' . $i . '" id="' . $i . '-txtTfinalMancomunado" value="' . $suma_total_mancomunado . '" readonly="readonly" class="validatasa"/></td>
										</tr>';
            $i++;
        }
        $resu->free();
