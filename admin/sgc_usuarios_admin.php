	...
	if(isset($_POST['accionUsuarios'])) {
		...
		if($errFlag) {
			...
			if($response){
			    ...
			    $mensaje="Hubo un error al ingresar los datos1, consulte con su administrador ".$conexion->errno.": ". $conexion->error;
		...
		if(isset($_POST['accionCambiar'])) {
			...
			if($errFlag) {
				...
				if($conexion->query($update)===TRUE){
				   ...
				   $mensaje="Hubo un error al ingresar los datos2, consulte con su administrador "."\n ".$conexion->errno. ": " . $conexion->error;
				   header('Location: index.php?l=usuarios_admin&op=2&msg='.$mensaje);
				} 
		...
		if(isset($_POST['accionCambiar'])) {
			...
			if($errFlag) {
				...
				if($conexion->query($update)===TRUE){
				   ...
				   $mensaje="Hubo un error al ingresar los datos3, consulte con su administrador "."\n ".$conexion->errno. ": " .$conexion->error;
				   header('Location: index.php?l=usuarios_admin&op=2&msg='.$mensaje);
				} 
			}
	...
	//NO PODEMOS ELIMINAR AL USUARIO ADMINISTRADOR
		...
		if(isset($_POST['btnBajaUsuario'])) {
			...
			if($conexion->query($update)===TRUE){
			    ...
			    $mensaje="Hubo un error al ingresar los datos4, consulte con su administrador "."\n ".$conexion->errno. ": " . $conexion->error;
			    header('Location: index.php?l=usuarios_admin&op=2&msg='.$mensaje);
			} 
