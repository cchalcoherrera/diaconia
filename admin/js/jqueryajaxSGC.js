...
$(function(){
	  $(".validatasa").blur(function(e){
			 ...
			 var id=vec[0];
			 var tipo=vec[1];
			 if(tipo=='txtTcompania' || tipo=='txtTcompaniaMancomunado'){
			 	 //alert(id);
				 var tasacompania = $('#'+id+'-txtTcompania').prop('value');
                                 var tasacompaniamancomunado = $('#'+id+'-txtTcompaniaMancomunado').prop('value');
				 //alert(tasacompania);
				 if (tasacompania=='' || tasacompaniamancomunado=='') {
				   //alert('hola');
				   e.preventDefault();
				   $("#errortcia"+id).html("ingrese la tasa");
				   $('#'+id+'-txtTcompania').css({'border' : '1px solid #d44d24'}).focus();
				   $("#btnSaveTasas").attr("disabled", true);
				 }else if(tasacompania.match(/^[0-9\.]+$/) && tasacompaniamancomunado.match(/^[0-9\.]+$/)){
					 ...
					 if(tasabanco==''){
	                     ...
	                     $('#'+id+'-txtTbanco').css({'border' : '1px solid #D1D1D1'});
						  ...
						  var suma = tasac + tasab;
                                                  ...
                                                  var tasacmancomunado = parseFloat(tasacompaniamancomunado);
                                                  var suma_mancomunado = tasacmancomunado + tasab;
						   //alert(suma);
						   $('#'+id+'-txtTfinal').prop('value',suma.toFixed(3)).hide().fadeIn("slow");
                                                   $('#'+id+'-txtTfinalMancomunado').prop('value',suma_mancomunado.toFixed(3)).hide().fadeIn("slow");
					 }else{
						 e.preventDefault();
