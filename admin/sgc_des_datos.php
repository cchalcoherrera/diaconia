										  ...
										  <thead>
											  <tr style="font-size:11.5px;">
												  ...
												  <th style="text-align:center;"><b><span lang="es">Parametros Adicionales</span></b></th>
                                                                                                  <th style="text-align:center;"><b><span lang="es">Parametros Adicionales Hipotecario</span></b></th>';
													if($tipo_sesion=='ROOT'){
												  ...
												  while($regi = $res->fetch_array(MYSQLI_ASSOC)){
													  echo'<tr style="font-size:11.5px;">
															  ...
															  <td style="text-align:center;">
															    <a href="?l=des_datos&idhome='.base64_encode($regi['id_home']).'&id_ef='.base64_encode($regief['id_ef']).'&parameters=v&var='.$_GET['var'].'&hipotecario=no" class="da-tooltip-s" title="Editar">Modificar Parametros</a>
															  </td>
                                                                                                                          <td style="text-align:center;">
															    <a href="?l=des_datos&idhome='.base64_encode($regi['id_home']).'&id_ef='.base64_encode($regief['id_ef']).'&parameters=v&var='.$_GET['var'].'&hipotecario=si" class="da-tooltip-s" title="Editar">Modificar Parametros Hipotecario</a>
															  </td>
                                                                                                                          ';
															  if($tipo_sesion=='ROOT'){
	  ...
	  $('#frmDatosAdmin').submit(function(e){
		  ...
		  if(sum==0){
			   ...
			   var idhome='<?=$_GET['idhome'];?>';
                           var hipotecario='<?=$_GET['hipotecario'];?>';
                           alert(hipotecario);
			   $.ajax({
					 async: true,
					 cache: false,
					 ...
					 data: FormCadena+'&opcion=guarda_parametros&id_ef='+id_ef+'&idhome='+idhome+'&hipotecario='+hipotecario,
					 beforeSend: function(){
	...
	if($rs = $conexion->query($select, MYSQLI_STORE_RESULT)){
			...
			if($num>0) {
				...
				$row = $rs->fetch_array(MYSQLI_ASSOC);
                                if($_GET['hipotecario']=="si"){
                                       $jsondata = $row['data_hip'];
                                }else{
                                       $jsondata = $row['data'];
                                }
