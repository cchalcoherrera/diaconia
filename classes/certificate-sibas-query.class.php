...
class CertificateQuery extends CertificateHtml {
    ...
    protected function __construct() {
        switch ($this->category) {
            case 'SC':  //	Slip de Cotización
                ...
                break;
            case 'CE':  //	Certificado
                $this->get_query_ce($this->category);
                break;
    ...
    //	CERTIFICADOS
    private function get_query_ce($categorie) {
        switch ($this->product) {
            case 'DE':
                if ($this->modality === false) {
                    if ($categorie == "CE") {
    ...
    private function set_query_de_sc() {  //DESGRAVAMEN
        $this->sqlPo = "select 
			...
			ssh.data,
			efc.id_ef_cia,
                         if(scc.fecha_creacion>='2017-02-01', scc.fecha_creacion,
                          if(scc.tipo_plazo='M', DATE_ADD(scc.fecha_creacion, INTERVAL scc.plazo MONTH), if(scc.tipo_plazo='D', DATE_ADD(scc.fecha_creacion, INTERVAL scc.plazo DAY), if(scc.tipo_plazo='Y', DATE_ADD(scc.fecha_creacion, INTERVAL scc.plazo YEAR), if(scc.tipo_plazo='W', DATE_ADD(scc.fecha_creacion, INTERVAL scc.plazo WEEK), scc.fecha_creacion))))
                          )as fecha_suma
		from
    ...
    private function set_query_de_em() { //DESGRAVAMEN
        $this->sqlPo = 'select 
			...
			@monto_s := (case sde.moneda
				...
				when "USD" then (((sde.monto_solicitado * 6.86) * sdd.porcentaje_credito) / 100)
			end) as monto_sol_bs,
			(case sde.tipo_plazo
				when "Y" then "años"
				when "D" then "dias"
			...
			sde.emitir,
			sde.fecha_emision,
                        sde.fecha_emision as fecha_real,
			sde.facultativo,
			...
			if(sde.moneda="USD", sdd.cumulo / (6.86), sdd.cumulo) as monto_acumulado_cliente,
                        agen.agencia as nombre_agencia,
		...
		from 
			...
			s_producto_cia as spc ON (spc.id_prcia = sde.id_prcia)
                                inner join
			s_agencia as agen ON (agen.id_agencia = su.id_agencia)
                ...
                $this->sqlDt = 'select 
					...
					(if(scl.ap_casada="",
					    concat(scl.nombre," ",scl.paterno," ",scl.materno),
						 if(scl.estado_civil="VIU", concat(scl.nombre," ",scl.paterno," ",scl.materno," viuda de ",scl.ap_casada),
							concat(scl.nombre," ",scl.paterno," ",scl.materno," de ",scl.ap_casada))
						 )) as nombre_completo,
					scl.genero,
