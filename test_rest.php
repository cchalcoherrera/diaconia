...
if(isset($_POST['ws-nt'])){
	...
	$flag = FALSE;
	/*
	$str = exec("ping -c 1 http://10.80.80.12");
	if ($result == 0){
	  //echo "ping succeeded";
	  $flag = TRUE;
	}else{
	...
	if($flag){
		...
		if($_POST['ws_cob']=='BC'){
		   //$str_datos = file_get_contents("http://10.80.70.33/wsodfclibc.php?parametro=".$_POST['ws-nt']);
                   $str_datos = file_get_contents("http://10.80.80.12/wsodfclibc.php?parametro=".$_POST['ws-nt']);
	    }elseif($_POST['ws_cob']=='IM'){
		   //$str_datos = file_get_contents("http://10.80.70.33/wsodfcliind.php?parametro=".$_POST['ws-nt']);
                   $str_datos = file_get_contents("http://10.80.80.12/wsodfcliind.php?parametro=".$_POST['ws-nt']);
		}
		//echo $str_datos;
