...
class ClientController extends Diaconia {
    ...
    public function getOccupationCaedec_ws($code) {
        $data = [];
    ...
    public function getOccupationCaedecCustomer($code) {
        $data = [];
            ...
            if (( $data['data'] = $this->$getDataProduct(base64_encode($data['idef'])) ) !== false) {
                ...
                for ($k = 0; $k < $nCl; $k++) {
                    if (isset($data['dc-name-' . $k], $data['dc-doc-id-' . $k])) {
                        ...
                        if ($this->verifyYearUser($data['data']['edad_min'], $data['data']['edad_max'], $temp_data['birth']) === false
                        ) {
                    ...
                    if ($flag) {
                        ...
                        if ($rec) {
                            if ($bc) {
                                ...
                                $mess[0] = 1;
                                $mess[1] = 'de-quote.php?ms=' . $ms . '&page=' . $page . '&pr=' . $pr . '&idc=' . base64_encode($data['idc']);
                                $mess[2] = 'Cliente registrado con exito!';
    ...
    public function getId_Prcia($idc, $idef) {
        $idc = $this->cx->real_escape_string(trim(base64_decode($idc)));
        $idef = $this->cx->real_escape_string(trim(base64_decode($idef)));

        $ClientRepo = new ClientRepo($this->cx);

        $data = $ClientRepo->getId_PrciaData($idc, $idef);
    ...
    public function getPrcia($idc, $idef) {
        $ClientRepo = new ClientRepo($this->cx);

        $idc = $this->cx->real_escape_string(trim(base64_decode($idc)));
        $idef = $this->cx->real_escape_string(trim(base64_decode($idef)));

        $data = $ClientRepo->getPrciaData($idc, $idef);

        return $data;
    }

    public function getFecha_Suma($idc, $idef) {
        $ClientRepo = new ClientRepo($this->cx);

        $idc = $this->cx->real_escape_string(trim(base64_decode($idc)));
        $idef = $this->cx->real_escape_string(trim(base64_decode($idef)));

        $data = $ClientRepo->getFecha_SumaData($idc, $idef);
