...
class QuoteController extends Diaconia {
    ...
    public function getResultQuote($idc, $idef, $numero_titulares) {
        $idc = $this->cx->real_escape_string(trim(base64_decode($idc)));
        $idef = $this->cx->real_escape_string(trim(base64_decode($idef)));

        $QuoteRepo = new QuoteRepo($this->cx);
        $data = $QuoteRepo->getResultQuoteData($idc, $idef, $numero_titulares);
    ...
    public function getResultTitulares($idc) {
        $idc = $this->cx->real_escape_string(trim(base64_decode($idc)));

        $QuoteRepo = new QuoteRepo($this->cx);
        $data = $QuoteRepo->getResultTitularesData($idc);
