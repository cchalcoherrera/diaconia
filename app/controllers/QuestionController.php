...
class QuestionController extends Diaconia {
    ...
    public function getQuestionHipotecario($idef, $product = 'DE') {
        $idef = $this->cx->real_escape_string(trim(base64_decode($idef)));
        $QuestionRepo = new QuestionRepo($this->cx);

        $data = $QuestionRepo->getQuestionDataHipotecario($idef, $product);
    ...
    public function postQuestion($data, &$mess) {
        ...
        if ($pr === 'DE|03') {
            ...
            if ($data['id_prcia'] == 7 and $data['fecha_suma'] >= "2017-02-01") {
            ...
            if (($data_pr = $this->getDataProduct(base64_encode($idef))) !== false) {
                ...
                if (($questions = $this->$functionName(base64_encode($idef))) !== false) {
                    for ($k = 1; $k <= $n_cl; $k++) {
                        $i = 0;
