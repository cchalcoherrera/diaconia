...
class PolicyController extends Diaconia {
    ...
    public function setLabelData($flag, &$sw, &$action, &$title, &$title_btn, &$read_new, &$read_save, &$read_edit) {
        switch ($flag) {
            case md5('i-new'):
    ...
    public function postPolicy($vars, &$mess) {
        ...
        if ($pr === 'DE|05') {
            ...
            if ($vars['id_prcia'] == 7) {
                $getDataProduct = "getDataProductHip";
            } else {
            ...
            switch ($flag) {
                case md5('i-new'): $sw = 1;
                    break;
                case md5('i-read'): $sw = 2;
                    break;
                case md5('i-edit'): $sw = 3;
                    break;
            }
            ...
            if ($n_cl > 0 && $n_cl <= $max_item) {
                ...
                foreach ($arr_cl as $key => &$client_data) {
                    if ($ClientController->verifyYearUser($data_pr['edad_min'], $data_pr['edad_max'], $client_data['cl-date']) === false) {
                ...
                if (isset($_POST['de-idc'])) {
                    $idc = $_POST['de-idc'];
                    $ide = "";
                } elseif (isset($_POST['de-ide'])) {
                    $ide = $_POST['de-ide'];
                    $idc = "";
                }

                $num_tirulares = $PolicyRepo->getResultTitularesData(base64_decode($idc), base64_decode($ide));
                $cobertura = $PolicyRepo->getResultCoberturaData(base64_decode($idc), base64_decode($ide));
                if ($cobertura == 1 and $num_tirulares == 2 and $vars['dcr_prcia']!=7) {
                    $TASA_TEMP = $PolicyRepo->getTasaMancomunado($vars['dcr_cia'], $vars['dcr_prcia'], $vars['idef']);
                }  else {
                $TASA_TEMP = $PolicyRepo->getTasa($vars['dcr_cia'], $vars['dcr_prcia'], $vars['idef']);    
                }
                ...
                if ($vars['dcr_prcia'] == 7) {
                    if ($num_tirulares == 1) {
                        $variacion = 1;
                    } elseif ($num_tirulares > 1) {
                        $variacion = 1;
                    }
                    $fn = $PolicyRepo->getFactorDescuento(base64_decode($idc), base64_decode($ide), $vars['idef'], $num_tirulares);
                    $TASA = ($TASA_TEMP * $num_tirulares) * ($variacion - $fn);
                } else {
                    if ($cobertura == 1 and $num_tirulares == 2) {
                        $TASA = $TASA_TEMP;
                    } else {
                        $TASA = $TASA_TEMP;
                    }
                ...
                foreach ($arr_cl as $key => &$client_data) {
                    ...
                    //OJOOOOOO
                    if ($vars['dcr_coverage'] === 2) {
                        $vars['amount'] = $client_data['cl-amount-bc'];
                        if ($vars['dcr_currency'] === 'USD') {
                            $vars['amount'] = $client_data['cl-amount-bc'] * $tcm;
                        }
                    } else {
                        $vars['amount'] = $client_data['cl-accumulation'];
                        if ($vars['dcr_currency'] === 'USD') {
                            $vars['amount'] = $client_data['cl-accumulation'] * $tcm;
                        }
                    }

                    foreach ($data_product['ranges'] as $key1 => $ranges) {
                        foreach ($ranges['range'] as $key2 => $range) {
                            if (($vars['amount'] >= $range['amount_min'] && $vars['amount'] <= $range['amount_max']) && ($age >= $range['edad_min'] && $age <= $range['edad_max'])) {
                                $slug = $ranges['slug'];
                                $token_range = true;
                                $amount_max_bs = $range['amount_min'];
                                break;
                            }
                        }
                    ...
                    if ($ClientController->getImc($client_data['cl-weight'], $client_data['cl-height']) && $slug !== 'FC') {
                        $IMC = true;
                    ...
                    if ($cu_amount === true) {
                        $CU = true;
                        $mess_aux = '
                            | El monto total acumulado del Titulars ' . $key . ' '
                        ...
                        $cu_amount_mess = '
                            ...
                            Monto maximo permitido ' . number_format($amount_max_bs, 2, '.', ',')
                                . ' BS. ';
                ...
                if ($birth_flag) {
                    if ($sw === 1 && $swPolicy === false) {
                        ...
                        if ($PolicyRepo->postPolicyData($vars, $arr_cl)) {
                            ...
                            $mess[2] = 'La Póliza no pudo ser registrada ' . $vars['de-idc'] . $PolicyRepo->postPolicyData2($vars, $arr_cl);
                        }
                    } elseif ($sw === 3) {
                        ...
                        if ($PolicyRepo->putPolicyData($vars, $arr_cl)) {
                            $mess[0] = 1;
                            if ($_POST['edit'] == "edit") {
                                $mess[1] = "index.php?ms=" . md5('MS_EDI') . "&page=" . md5('P_general');
                            } else {
                                $mess[1] = 'de-quote.php?ms=' . $ms . '&page=' . $page
                                        . '&pr=' . $pr . '&ide=' . base64_encode($vars['id'])
                                        . '&flag=' . md5('i-read') . '&cia=' . base64_encode($vars['dcr_cia'])
    ...
    private function setPolicyData(&$vars, $data_product) {
        ...
        if (array_key_exists('policies', $data_product)) {
            foreach ($data_product['policies'] as $key => $policy) {
                if ($policy['currency'] === $vars['dcr_currency'] && $policy['active']) {
    ...
    public function setClientData(&$vars, &$arr_cl, &$client, $n_cl) {
        ...
        while ($k < $n_cl) {
            ...
            if (empty($aux_occupation) === false) {
                $arr_cl[$k]['cl-occupation'] = '"' . $this->cx->real_escape_string(trim(base64_decode($aux_occupation))) . '"';
            }
            ...
            $arr_cl[$k]['cl-residue'] = $this->cx->real_escape_string(trim($vars['dcr-amount-de-' . $k]));
            $arr_cl[$k]['cl-accumulation'] = $this->cx->real_escape_string(trim($vars['dcr-amount-ac-' . $k]));
