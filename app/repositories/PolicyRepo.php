...
class PolicyRepo {
    ...
    public function getQuoteData($idc, $idef) {
        ...
        $sql = 'select 
            ...
            sdc.id_pr_extra,
            sdc.modalidad,
                if(sdc.fecha_creacion>="2017-02-01", sdc.fecha_creacion,
            if(sdc.tipo_plazo="M", DATE_ADD(sdc.fecha_creacion, INTERVAL sdc.plazo MONTH), if(sdc.tipo_plazo="D", DATE_ADD(sdc.fecha_creacion, INTERVAL sdc.plazo DAY), if(sdc.tipo_plazo="Y", DATE_ADD(sdc.fecha_creacion, INTERVAL sdc.plazo YEAR), if(sdc.tipo_plazo="W", DATE_ADD(sdc.fecha_creacion, INTERVAL sdc.plazo WEEK), sdc.fecha_creacion))))
            ) as fecha_suma
        from
    ...
    public function getTasa($cia, $pr_cr, $idef, $product = 'DE') {
        ...
        public function getTasaMancomunado($cia, $pr_cr, $idef, $product = 'DE') {
        $sql = 'select 
            spc.id_prcia, st.tasa_final_mancomunado
        from
            s_tasa_de as st
                inner join
            s_producto_cia as spc ON (spc.id_prcia = st.id_prcia)
        ...
        if (($rs = $this->cx->query($sql, MYSQLI_STORE_RESULT)) !== false) {
            if ($rs->num_rows === 1) {
                $row = $rs->fetch_array(MYSQLI_ASSOC);
                $rs->free();
                return $row['tasa_final_mancomunado'];
            }
        }
    ...
    public function getFactorDescuento($idc, $ide, $idef, $numero_titulares) {
        $data = array();
        if ($idc!="") {
            $sql = 'select 
                        if(sdc.id_prcia="7", (SELECT des.descuento as des FROM s_tasa_factor_descuento as des WHERE des.id_prcia=sdc.id_prcia AND "' . $numero_titulares . '" between min_titulares and max_titulares ), 0) as factor_descuento
		from
			s_de_cot_cabecera as sdc
				inner join 
			s_de_cot_detalle as sdd ON (sdd.id_cotizacion = sdc.id_cotizacion)
    ...
    public function getResultTitularesData($idc, $ide) {
        $data = 0;
        if ($idc!="") {
            $sql = 'select 
			COUNT(*) as numero_titulares
		from
			s_de_cot_cabecera as sdc
				inner join 
			s_de_cot_detalle as sdd ON (sdd.id_cotizacion = sdc.id_cotizacion)
				inner join
			s_de_cot_cliente as cli ON (cli.id_cliente = sdd.id_cliente)
		where
			sdc.id_cotizacion = "' . $idc . '"
		;';
        } elseif ($ide!="") {
		...
		from
			...
			s_de_em_detalle as sdd ON (sdd.id_emision = sdc.id_emision)
				inner join
			s_cliente as cli ON (cli.id_cliente = sdd.id_cliente)
		where
			sdc.id_emision = "' . $ide . '"
		;';
        }
        ...
        public function getResultCoberturaData($idc, $ide) {
        $data = 0;
        if ($idc!="") {
            $sql = 'select 
			cobertura
    ...
    public function postPolicyData($vars, $arr_cl) {
        ...
        ("' . $vars['id'] . '", "' . $vars['record'] . '", 
            ...
            "0000-00-00", "", false, "0000-00-00", "' . $vars['dcr_cia'] . '", 
    ...
    public function postPolicyData2($vars, $arr_cl) {
        $detail = true;
