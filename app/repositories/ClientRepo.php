...
class ClientRepo
{
    ...
    public function getClientData($idc, $idef, $idcl)
    {
     ...
     public function getId_PrciaData($idc, $idef)
    {
        $data = [ ];
        ...
        $sql = 'select
			scl.id_prcia
		from
			s_de_cot_cabecera as scl
		where
				scl.id_cotizacion = "' . $idc . '"
		;';
        //echo $sql;
        if (( $rs = $this->cx->query($sql, MYSQLI_STORE_RESULT) ) !== false) {
            if ($rs->num_rows === 1) {
    ...
    public function getListClientData($idc, $idef, $max_item)
    {
      ...
      public function getFecha_SumaData($idc, $idef) {

        $sql = "select
            if(sdc.fecha_creacion>='2017-02-01', sdc.fecha_creacion,
                        if(sdc.tipo_plazo='M', DATE_ADD(sdc.fecha_creacion, INTERVAL sdc.plazo MONTH), if(sdc.tipo_plazo='D', DATE_ADD(sdc.fecha_creacion, INTERVAL sdc.plazo DAY), if(sdc.tipo_plazo='Y', DATE_ADD(sdc.fecha_creacion, INTERVAL sdc.plazo YEAR), if(sdc.tipo_plazo='W', DATE_ADD(sdc.fecha_creacion, INTERVAL sdc.plazo WEEK), fecha_creacion))))
                        )as fecha_suma
		from
			s_de_cot_cabecera as sdc
				inner join
    ...
    public function getPrciaData($idc, $idef) {
        $data = [];
