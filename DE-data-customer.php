...
if ($swCl === false) {
    $id_prcia = $ClientController->getId_Prcia($_GET['idc'], $_SESSION['idEF'], $max_item);
    $data_list = $ClientController->getListClient($_GET['idc'], $_SESSION['idEF'], $max_item);
    $nCl = count($data_list);
    if ($id_prcia['id_prcia'] != 7 && $id_prcia['id_prcia'] != 1) {
        $max_item = 2;
    }
...
<form id="fde-customer" name="fde-customer" action="" method="post" class="form-quote form-customer">
    ...
    if ($nCl < $max_item || $swCl === true) {
                ...
                <table class="list-cl">
                    <thead>
                        <tr>
                            ...
                            <td style="width:15%;">Codigo CAEDEC</td>
                                ...
                                <td>
                                    <script type="text/javascript">
                                        $(document).ready(function(e) {
                                            $('#dc-<?= $k; ?>-carga').hide();
                                            $('#dc-occupation-<?= $k ?>').prop('disabled', false);
                                            $("#dc-<?= $k; ?>-edit").click(function() {
                                                $('#dc-occupation-<?= $k ?>').prop('disabled', false);
                                                $('#dc-<?= $k; ?>-desc-occ').val('');
                                                $('#dc-<?= $k; ?>-carga').show();
                                                $('#dc-occupation-<?= $k ?>').hide();
                                                $.ajax({
                                                    type: "POST",
                                                    url: "getOcupacion.php?count=<?= $k; ?>",
                                                    success: function(response)
                                                    {
                                                        $('#dc-occupation-<?= $k ?>').html(response).fadeIn();
                                                        $('#dc-<?= $k; ?>-carga').hide();
                                                        $('#dc-occupation-<?= $k ?>').show();
                                                    }
                                                });
                                            });
                                        });
                                    </script><div id="dc-<?= $k; ?>-edit"><img src="img/edit-inf-icon.png" width="20" height="20" alt="Editar" title="Editar" style="float:left; padding-top: 5px; cursor: pointer; display:<?= $show; ?>" ></div>
                                    <div id="dc-<?= $k; ?>-carga" style="float: left;">
                                        <img src="img/loading-01.gif" width="25" height="25" alt="Cargando" title="Cargando">
                                    </div>
                                    <select id="dc-occupation-<?= $k ?>" name="dc-occupation-<?= $k ?>" class="fbin occupation" style="width: 145px;">
                                        <?php
                                        $ocupacion = $ClientController->getOccupationCaedecCustomer($data['occupation']);
                                        $selected = ''
                                        ?>
                                        <option selected="selected" value="<?= base64_encode($ocupacion[0]['id_ocupacion']); ?>" 
                                                data-desc="<?= trim($ocupacion[0]['ocupacion']); ?>" data-number="<?= $cont; ?>"
                                                >Código CAEDEC - <?= $data['caedec'] ?>
                                        </option>
                                    </select>
                                </td>
                                <td>
                ...
                <div class="form-col">
                    ...
                    <div class="content-input">
                        <input type="text" id="dc-ln-patern-<?= $k ?>" name="dc-ln-patern-<?= $k ?>"
                               autocomplete="off" value="<?= $data['patern']; ?>" class="fbin">
                    </div>
                    <br>
                ...
                <div class="form-col">
                    ...
                    <div class="content-input">
                        <input type="text" id="dc-phone1-<?= $k ?>" name="dc-phone1-<?= $k ?>"
                               autocomplete="off" value="<?= $data['phone_1']; ?>" class="not-required number fbin">
                    </div>
                    <br>
                    ...
                    <div class="content-input">
                        <input type="text" id="dc-phone2-<?= $k ?>" name="dc-phone2-<?= $k ?>"
                               autocomplete="off" value="<?= $data['phone_2']; ?>" class="not-required number fbin">
                    </div>
                    <br>
                    ...
                    <?php if (!$lists): ?>
                        ...
                        <div class="content-input">
                            ...
                            </script><div id="dc-<?= $k; ?>-edit"><img src="img/edit-inf-icon.png" width="25" height="25" alt="Editar" title="Editar" style="float:left; padding-top: 5px; cursor: pointer; display:<?= $show; ?>" ></div>
                            <div id="dc-<?= $k; ?>-carga" style="float: left;">
                                <img src="img/loading-01.gif" width="25" height="25" alt="Cargando" title="Cargando">
                            </div>
                            <select id="dc-occupation-<?= $k ?>" name="dc-occupation-<?= $k ?>" class="fbin occupation" style="width: 150px;">
                                <option value="">Seleccione...</option>
                                <?php
                                $ocupacion = $ClientController->getOccupationCaedecCustomer($data['occupation']);
                                $selected = ''
                    ...
                    <textarea readonly="readonly" id="dc-desc-occ-<?= $k ?>" name="dc-desc-occ-<?= $k ?>"
        ...
        <input type="hidden" id="id_prcia" name="id_prcia" value="<?= $id_prcia['id_prcia']; ?>">
        <?php if ($swCl): ?>
