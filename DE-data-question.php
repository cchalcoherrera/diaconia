...
$producto_crediticio = $ClientController->getPrcia($_GET['idc'], $_SESSION['idEF']);
$fecha_suma = $ClientController->getFecha_Suma($_GET['idc'], $_SESSION['idEF']);


if ($producto_crediticio == 7 and $fecha_suma >= "2017-02-01") {
    ...
    if (($data = $Diaconia->$getDataProduct($_SESSION['idEF'])) !== false) {
        ...
        if (count($clients) > 0) {
            ...
            <form id="fde-question" name="fde-question" action="" method="post" class="form-quote form-question">
                <?php
                echo '<input type="hidden" name="id_prcia" value="' . $producto_crediticio . '">';
                echo '<input type="hidden" name="fecha_suma" value="' . $fecha_suma . '">';
