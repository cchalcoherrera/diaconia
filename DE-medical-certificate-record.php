...
if ((isset($_POST['fc-cm-1']) || isset($_POST['fc-cm-2']) || isset($_POST['fc-cm-3'])) && isset($_POST['ide']) && (isset($_POST['fc-idCl-1']) || isset($_POST['fc-idCl-2']) || isset($_POST['fc-idCl-3'])) && isset($_POST['fp-idCm'])) {
    $link = new SibasDB();
    ...
    if (isset($_POST['fc-cm-3'])) {
        $cm3 = (int) $link->real_escape_string(trim($_POST['fc-cm-3']));
        $k = 3;
    }
    if ($token === FALSE) {
        if ($k > 0) {
            ...
            if (count($arrCM) > 0) {
                ...
                for ($j = 1; $j <= count($arrCM); $j++) {
                    if (count($arrCM) === 1) {
                        if ($cm1 === 1 && $cm2 === 0 && $cm3 === 0)
                            $kk = 1;
                        elseif ($cm1 === 0 && $cm2 === 1 && $cm3 === 0)
                            $kk = 2;
                        elseif ($cm1 === 0 && $cm2 === 0 && $cm3 === 1)
                            $kk = 3;
                    }else {
                        $kk = $j;
                    }
