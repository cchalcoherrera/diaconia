...
$selectdes="SELECT t0.no_emision, t2.nombre, t2.paterno, t2.materno, t2.ci, case t2.genero when 'F' then 'Femenino' when 'M' then 'Masculino' end as genero, 
    t2.localidad, t2.telefono_celular, t2.telefono_domicilio, t2.email, t2.estatura, t2.peso, t2.edad, t0.motivo_facultativo, datediff(now(),
    t0.fecha_creacion) AS proceso, t0.tasa, if(t0.cobertura=1, t0.monto_solicitado, t1.monto_banca_comunal) as monto_solicitado, 
    t0.moneda, t0.cumulo_deudor, t0.cumulo_codeudor, t0.fecha_creacion, 
    case t7.respuesta when 0 then 'Observado' when 1 then 'Subsanado/Pendiente' else 'Pendiente' end as estado, t8.estado as observacion, t5.agencia, 
    t3.usuario, t3.nombre as nom_us, t3.email as email_us, case t0.aprobado when 1 then 'SI' when 0 then 'NO' end as aprobado, t0.prefijo,
    case t0.cobertura when '1' then 'Individual/Mancomunado' when '2' then 'Banca Comunal' end as cobertura
				...
				while($resutadodes=mysql_fetch_array($consultades)){
					...
					if($resutadodes['aprobado']=='SI'){$bg='background:#1a834c; color:FFF;'; }else{$bg='background:#f82d2d; color:FFF;'; }
						 ...
						 $shtml.='<td align="center">'.$resutadodes['prefijo'].'-'.$resutadodes['no_emision'].'</td>
                                                          <td align="center">' . $resutadodes['cobertura'] . '</td>
