...
$selectdes = "SELECT t0.no_emision, 
    t2.nombre, t2.paterno, t2.materno, t2.ci, case t2.genero when 'F' then 'Femenino' when 'M' then 'Masculino' end as genero, t2.lugar_residencia, 
    t2.localidad, t2.telefono_celular, t2.telefono_domicilio, t2.email, 
    if(t0.cobertura=1, t0.monto_solicitado, t1.monto_banca_comunal) as monto_solicitado, 
    t0.moneda, t0.plazo, 
    case t0.tipo_plazo when 'Y' then 'Años' when 'M' then 'Meses' when 'W' then 'Semanas' when 'D' then 'Dias' end as tipo_plazo, t2.estatura, t2.peso, 
    t1.porcentaje_credito, case t1.titular when 'DD' then 'Titular' when 'CC' then 'Codeudor' end as titular, t2.edad, t4.departamento, t0.tasa, 
    case t0.emitir when 1 then 'SI' when 0 then 'NO' end as emitir, t0.fecha_emision, t5.agencia, t3.usuario, t3.nombre as nom_us, t3.email as email_us, 
    t0.prefijo, case t0.cobertura when '1' then 'Individual/Mancomunado' when '2' then 'Banca Comunal' end as cobertura
